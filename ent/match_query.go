...
func (mq *MatchQuery) QueryMap() *OsnMapQuery {
	query := (&OsnMapClient{config: mq.config}).Query()
...
func (mq *MatchQuery) QueryRoles() *PlayerRoleQuery {
	query := (&PlayerRoleClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	if query := mq.withMap; query != nil {
		if err := mq.loadMap(ctx, query, nodes, nil,
			func(n *Match, e *OsnMap) { n.Edges.Map = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withRoles; query != nil {
		if err := mq.loadRoles(ctx, query, nodes,
			func(n *Match) { n.Edges.Roles = []*PlayerRole{} },
			func(n *Match, e *PlayerRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MatchQuery) loadMap(ctx context.Context, query *OsnMapQuery, nodes []*Match, init func(*Match), assign func(*Match, *OsnMap)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Match)
	for i := range nodes {
		if nodes[i].osn_map_matches == nil {
			continue
		}
		fk := *nodes[i].osn_map_matches
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "osn_map_matches" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MatchQuery) loadRoles(ctx context.Context, query *PlayerRoleQuery, nodes []*Match, init func(*Match), assign func(*Match, *PlayerRole)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(match.RolesTable)
		s.Join(joinT).On(s.C(playerrole.FieldID), joinT.C(match.RolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(match.RolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(match.RolesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*PlayerRole](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
