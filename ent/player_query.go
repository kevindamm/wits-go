...
func (pq *PlayerQuery) QueryRoles() *PlayerRoleQuery {
	query := (&PlayerRoleClient{config: pq.config}).Query()
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withRoles; query != nil {
		if err := pq.loadRoles(ctx, query, nodes,
			func(n *Player) { n.Edges.Roles = []*PlayerRole{} },
			func(n *Player, e *PlayerRole) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadRoles(ctx context.Context, query *PlayerRoleQuery, nodes []*Player, init func(*Player), assign func(*Player, *PlayerRole)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(player.RolesTable)
		s.Join(joinT).On(s.C(playerrole.FieldID), joinT.C(player.RolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(player.RolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(player.RolesPrimaryKey[1]))
