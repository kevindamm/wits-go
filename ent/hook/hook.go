...
// The OsnMapFunc type is an adapter to allow the use of ordinary
// function as OsnMap mutator.
type OsnMapFunc func(context.Context, *ent.OsnMapMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OsnMapFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OsnMapMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OsnMapMutation", m)
}
